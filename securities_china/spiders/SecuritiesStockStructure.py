...
class SecuritiesStockStructure(scrapy.Spider):
    ...
    def parse(self, response):
        ...
        links = response.xpath(
            "/html/body/div[2]/div[1]/table[2]/tr/td/a/@href").extract()
        links = links + response.xpath(
            "/html/body/div[2]/div[1]/table[2]/td/a/@href").extract()
        for link in links:
    ...
    def parseStockStructure(self, response):
        ...
        times = [time for time in trs[0].xpath("td/text()").extract()
                 if self.time_rexp.match(time) is not None]
        for i in range(len(times)):
            values = [value
                      for value in [trs[part[1] - 1].xpath("td[" + str(
                          i + 1) + "]/text()").extract()[0]
                                    for part in self.parts]]
            values = [value.replace(",", "") for value in values]
            values = [unicodedata.normalize("NFKD", value) for value in values]
            values = [values[j] if j < 3 else values[j].replace(" ", "0.00")
