...
class SecuritiesDividendPlan (scrapy.Spider):
    ...
    def __init__(self, years=None, codes=None):
        ...
        if(self.codes!=None):
            self.start_urls = [
                self.url_tpl + code for code in self.codes
            ]
        else:
            self.start_urls = [
                self.url_tpl + code[0] for code in self.db.query_securities_code().fetch_row(0)
            ]
    ...
    def merge_rows(self, response):
        ...
        it = itertools.groupby(self.parse_rows(response),operator.itemgetter(1))
    ...
    def parse_rows(self, response):
        ...
        for i in range(2, len(rows)):
            ...
            if(self.years==None or year in self.years):
