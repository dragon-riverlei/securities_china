...
class SecuritiesMajorFinancialKPI (scrapy.Spider):
    ...
    start_urls = [
        url_tpl + code[0] for code in db.query_securities_code().fetch_row(maxrows=0)
    ]
    ...
    def __init__(self, dates=None):
        if(dates!=None):
            ...
            self.years = { date[:4] for date in self.dates if self.time_rexp.match(date) != None }
        else:
            self.dates = None
            self.years = None

    def parse(self, response):
        links = response.xpath("/html/body/div[2]/div/table[2]/tr[1]/td/a/@href").extract()
        if(self.years != None):
            links = [link for link in links if link.split("type=")[1] in self.years ]
        for link in links:
    ...
    def eachYear(self, response):
        ...
        if(self.dates != None):
            chosenTimes = [ time for time in chosenTimes if time in self.dates]
        for time in chosenTimes:
            i = times.index(time)
            values = [ re.sub(self.unit_rexp, "", value)
                       for value in [
                                trs[kpi[2]-1].xpath("td[ " + str(i+1) + "]/text()").extract()[0] for kpi in self.kpis
                       ]
            ]
    ...
    def kpiNames(self, response):
        """Print all the kpi names found in the url to the file named after the securities code.
           This is to help verify if all the securties share the same set of kpis.
        """
        code = response.url.split("zqdm=")[1].split("&type=")[0].strip()
        f = open(code, "w")
        for kpi in response.xpath("/html/body/div[2]/div/table[3]/tr/th/a/text()").extract():
